...
A browser-based, 2.5D car track builder inspired by classic Hot Wheels sets and Rollercoaster Tycoon. Snap orange track pieces together on a grid, drop your car from a tower or a booster, and watch it tear through loops, corkscrews, and jumps. Static site, no build step — runs straight from GitHub Pages.

## Play

GitHub Pages: enable Pages on this repo with the included workflow (Settings → Pages → "GitHub Actions") and push. Once it deploys, the URL appears in the action summary.
...
├── editor.js                  build-mode UI (palette, ghost preview, undo)
├── pieces/
│   ├── geometry.js            DIRS, applyPiece, localToWorld
│   ├── paths.js               parametric path samplers per piece
│   ├── definitions.js         PIECES catalogue + palette ordering
│   ├── sampling.js            world-space samples used by physics + renderer
│   └── index.js               aggregator
├── renderer/
│   ├── index.js               Three.js scene, camera, run loop
│   ├── colors.js              palette
│   ├── meshes.js              piece + ghost + start-tower mesh builders
│   ├── car.js                 car mesh and tangent/banking placement
│   └── controls.js            drag / wheel / R-key camera controls
└── app/
    ├── hud.js                 HUD updates
    ├── overlay.js             run-result overlay
    └── storage.js             localStorage save/load

test/
├── pieces.geometry.test.js
├── pieces.paths.test.js
├── track.test.js
├── physics.test.js
└── scoring.test.js
```
...
`physics.js` uses an energy formulation: it tracks `v²` and updates it per substep with
